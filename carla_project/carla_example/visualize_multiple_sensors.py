...
def run_simulation(args, client):
    ...
    try:
        ...
        if args.sync:
            traffic_manager = client.get_trafficmanager(8000) # 管理非玩家角色（NPC）的交通行为
            settings = world.get_settings()
            traffic_manager.set_synchronous_mode(True)        # NPC 的行为将与主车辆同步。
            settings.synchronous_mode = True
            settings.fixed_delta_seconds = 0.05               # “tick”或“帧”  50ms  高精度模拟=10ms
            world.apply_settings(settings)


        # Instanciating the vehicle to which we attached the sensors # 创建一个车辆实例，并将其设置为自动驾驶模式
