...
from carla_example.dynamic_weather import set_custom_weather, set_rain_snow_weather
...
def run_simulation_Town10(client):
    ...
    try:
        if carla_config.Server["sync"]:  # 同步模式
            ...
            spectator.set_transform(carla.Transform(carla.Location(x=106.56, y=110.7, z=15), carla.Rotation(pitch = -15,yaw=-90)))
...
def spawn_crossbike(world, blueprint_library, g_vehicle_list, vehicle_type,x, y, z=0.6, yaw=90.439095):
    """
...
def main_run_carla(world, TM, g_vehicle_list):
    ...
    """获取实际刷出位置"""
    print("前方卡车位置:37 ", spawn_points[37])
    print("前方卡车位置:40 ", spawn_points[40])
    print("前方卡车位置:39 ", spawn_points[39])
    print("前方卡车位置:81 ", spawn_points[81])
    前方卡车位置: 81
    # Transform(Location(x=99.078560, y=42.141800, z=0.600000), Rotation(pitch=0.000000, yaw=90.390709, roll=0.000000))
    print("前方卡车位置:70 ", spawn_points[70])
...
前方卡车位置:43  Transform(Location(x=409.784119, y=-149.469055, z=0.281942), Rotation(pitch=0.000000, yaw=-89.401421, roll=0.000000))
    ...
    vehicle_type_to_indices = {
        ...
        'vehicle.tesla.model3': [73, 39,143],
        'vehicle.ford.mustang': [152,],
        'vehicle.toyota.prius': [100,72 ],
        'vehicle.bh.crossbike': [70],
        'vehicle.bmw.grandtourer':[103,148,],
    }
    ...
    vehicle_positions = [
        ...
        # # 隔壁车道车辆
        #
        # # (381.481506, -154.471375,  90),x=106.028816, y=67.419983,
        ('vehicle.bmw.grandtourer',       106, 30,    0.6,  -90),
        ('vehicle.jeep.wrangler_rubicon', 109, 27.,   0.3, -90),
        ...
        # ('vehicle.carlamotors.european_hgv', 108,-0, -90),
        ('walker.pedestrian.0029',111.2,   68,  0.9, -90),


        ('walker.pedestrian.0015',112.0,   60,   0.9, -80),
        ('walker.pedestrian.0016',112.0,   55,   0.9, -70),
        ('walker.pedestrian.0017',113.0,   52,   0.9, -30),
        ('walker.pedestrian.0027',112.0,   50,   0.9, -68),



        # ('walker.pedestrian.0001',93.0,   55,   0.9, -85),#左前
        # ('walker.pedestrian.0002',94.0,   50,   0.9, -90),
        # ('walker.pedestrian.0003',92.0,   45,   0.9, 90),
    ...
    """ego左侧自行车布置"""
    spawn_crossbike(world, blueprint_library, g_vehicle_list, "vehicle.bh.crossbike",x=98.078560, y=42.141800, z=0.600000)
    spawn_crossbike(world, blueprint_library, g_vehicle_list, "vehicle.bh.crossbike",x=99.078560, y=52.141800, z=0.600000)
    ...
    car_C_transform = carla.Transform(carla.Location(x=109.028816, y=67.419983, z=0.6),
