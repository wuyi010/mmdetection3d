...
# 粒子群优化算法类from mpl_toolkits.mplot3d import Axes3D
#
class PSO:
    ...
    def plot_particles(self, iter_count):
        ...
        ax.scatter(self.global_best_position[0], self.global_best_position[1], self.global_best_position[2], color='red', label='Global Best', marker='*', s=200)
        ax.set_xlim(self.bounds[0])
        ax.set_ylim(self.bounds[1])
        ax.set_zlim(self.bounds[2])
        ax.set_title(f'Iteration {iter_count + 1}')
        ax.legend()
        plt.draw()  # 绘制图像
...
"""
说明
plt.ion() 和 plt.ioff() 用于开启和关闭交互模式，以便可以动态更新图像。
plt.clf() 用于清空当前图像，以便每次迭代都能绘制新的图像。
plt.draw() 和 plt.pause() 用于显示图像并控制更新频率。
这样，你就可以在三维空间中动态查看粒子的位置，并且随着迭代过程的进行
"""